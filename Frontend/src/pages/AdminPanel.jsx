...
const AdminPanel = () => {
  ...
  const handleDropdownItemClick = (component, group) => {
    setActiveComponent(component);

    setIsProductDropdownOpen(group === "product");
    setIsUserDropdownOpen(group === "user");
    setIsOrderDropdownOpen(group === "order");
  };
                        ...
                        {productManagementItems.map((item, subIndex) => (
                          <li key={subIndex}>
                            <button
                              ...
                              onClick={() =>
                                handleDropdownItemClick(item.component, "product")
                              }
                            >
                              {item.label}
                            </button>
                        ...
                        {userManagementItems.map((item, subIndex) => (
                          <li key={subIndex}>
                            <button
                              className="flex items-center w-full px-4 py-2 text-left text-white hover:bg-gray-600 rounded-md transition duration-300 ease-in-out transform hover:scale-105 bg-gray-900"
                              onClick={() =>
                                handleDropdownItemClick(item.component, "user")
                              }
                            >
                              {item.label}
                            </button>
                        ...
                        {orderManagementItems.map((item, subIndex) => (
                          <li key={subIndex}>
                            <button
                              className="flex items-center w-full px-4 py-2 text-left text-white hover:bg-gray-600 rounded-md transition duration-300 ease-in-out transform hover:scale-105 bg-gray-900"
                              onClick={() =>
                                handleDropdownItemClick(item.component, "order")
                              }
                            >
                              {item.label}
                            </button>
